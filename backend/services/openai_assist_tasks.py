        ...
        *,
        ...
        try:
            data = self._create_multimodal_response(
                api_key=settings["apiKey"],
                model=settings["modelName"],
                instructions=BENCH_PHOTO_VERIFICATION_INSTRUCTIONS,
                input_text=build_bench_photo_verification_prompt(
                    plan=plan,
                    step=step,
                    note=note,
            ...
            self._record_ai_failure(event_base, message, latency_ms=int((time.time() - started_at) * 1000))
            if self.logger:
                self.logger.warning(f"OpenAI bench photo verification failed: {message}")
            return None

    def triage_project_finder(
        self,
        *,
        ...
        try:
            data = self._create_response(
                api_key=settings["apiKey"],
                model=settings["modelName"],
                instructions=PROJECT_FINDER_TRIAGE_INSTRUCTIONS,
                input_text=build_project_finder_triage_prompt(
            ...
            self._record_ai_failure(event_base, message, latency_ms=int((time.time() - started_at) * 1000))
            if self.logger:
                self.logger.warning(f"OpenAI Project Finder triage failed: {message}")
            return None

    def enrich_circuit_graph(
        self,
        *,
        plan: dict[str, Any],
        graph: dict[str, Any],
        ...
        try:
            data = self._create_response(
                api_key=settings["apiKey"],
                model=settings["modelName"],
                instructions=CIRCUIT_GRAPH_ENRICHMENT_INSTRUCTIONS,
                input_text=build_circuit_graph_enrichment_prompt(
                    plan=plan,
                    graph=graph,
                    local_review=local_review,
                ),
