...
SETTING_UI = {
    ...
    "QUERY_RETRIES": ("server", "Query retries", "How many times to retry a failed model request.", False),
    "QUERY_RETRY_DELAY": ("server", "Retry delay", "Seconds to wait between failed model request retries.", False),
    ...
    "LOCAL_GPU_OCR_SLOTS": ("models", "OCR CUDA lanes", "PaddleOCR CUDA work lanes. Auto sizes from detected GPU count and VRAM, with 20GB+ GPUs allowed higher OCR concurrency.", False),
    ...
    "LLM_NUM_CTX": ("models", "Context window", "Maximum context tokens requested from the Ollama model.", False),
    ...
    "INGEST_LOCAL_AI_REVIEW_ENABLED": ("ingestion", "Local ingestion review", "Use the local Ollama model as the first ingestion QA pass when deterministic extraction detects component or datasheet risk.", False),
    "INGEST_LOCAL_AI_MAX_PENDING": ("ingestion", "Local ingestion AI backlog", "Maximum live local LLM ingestion-review jobs allowed at once, including one running job. Keeps large ingests from flooding the GPU queue.", False),
    "INGEST_LOCAL_AI_ADMISSION_TIMEOUT_SECONDS": ("ingestion", "Local ingestion AI wait", "Seconds an ingestion review may wait before entering the local LLM queue when the local GPU lane is busy.", False),
    ...
    "RESPONSE_CACHE_CAPACITY": ("retrieval", "Response cache size", "Maximum number of cached query responses kept by the Postgres-backed response cache.", True),
    ...
    "RESPONSE_FINALIZER_MODE": ("retrieval", "Validation mode", "When to run answer validation: off, always, auto, issues, build, build_or_issues, low_confidence, build_or_low_confidence, or issues_or_build_or_low_confidence.", False),
    ...
    "RESPONSE_FINALIZER_MAX_CONTEXT_CHARS": ("retrieval", "Validation context limit", "Maximum source-summary characters sent to the response validator.", False),
    "RERANK_MAX_CONTEXT_CHUNKS": ("retrieval", "Answer context chunks", "Maximum reranked text chunks sent into one RAG answer prompt after cross-encoder scoring.", False),
    ...
    "OCR_ENGINE": ("ocr", "OCR engine", "Choose Tesseract CPU for stable OCR or PaddleOCR GPU for faster OCR with automatic Tesseract fallback.", False),
    "OCR_ENGINE_FALLBACK": ("ocr", "OCR engine fallback", "Internal safety setting. PaddleOCR always falls back to Tesseract when unavailable or failed.", True),
    "PADDLEOCR_DEVICE": ("ocr", "PaddleOCR compute device", "Internal setting. CircuitShelf uses PaddleOCR on GPU only; CPU PaddleOCR is not exposed because this host falls back to Tesseract.", True),
    "PADDLEOCR_LANG": ("ocr", "PaddleOCR language", "PaddleOCR recognition language code.", True),
