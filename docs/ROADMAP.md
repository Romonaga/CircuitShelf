...
- Recovery work after FastDrive loss
  ...
  - In progress: Split `circuitshelf.py` into modular backend domains. App factory, route registration, auth dependencies, API routers, services, stores, server startup helpers, settings/environment bootstrap, source metadata, document intelligence, document management, prompt building, Ollama chat, image retrieval, retrieval helpers, RAG orchestration, ingest statistics, and runtime status reporting are separated; ingestion orchestration still needs deeper extraction.
